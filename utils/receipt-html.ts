...
// Dragonfly Logo SVG Component
const getDragonflySvg = (): string => `
...
const generateDetailsHtml = (
  transactionDetails: { label: string; value: string; isAmount?: boolean }[] | undefined,
  mainColor: string
): string => {
  ...
  return transactionDetails.map(detail => `
    <div style="display: flex; justify-content: space-between; margin-bottom: 4px; border-bottom: 1px solid #f3f4f6; padding-bottom: 3px;">
      ...
      <span style="font-weight: ${detail.isAmount ? '700' : '500'}; font-size: ${detail.isAmount ? '16px' : '12px'}; color: ${detail.isAmount ? mainColor : '#111827'};">
        ${detail.value}
      </span>
    </div>
  `).join('');
...
// Generate CSS styles for receipt
const getReceiptStyles = (iconBgColor: string): string => `
...
// Generate status icon SVG
const getStatusIcon = (isSuccess: boolean): string => {
  if (isSuccess) {
...
export const generateReceiptHtml = async ({ result, card: _card, transactionDetails, theme }: ReceiptData) => {
  ...
  const dragonflySvg = getDragonflySvg();
  ...
  const detailsHtml = generateDetailsHtml(transactionDetails, theme.tenant.mainColor);
  const statusIcon = getStatusIcon(isSuccess);
  const styles = getReceiptStyles(iconBgColor);
