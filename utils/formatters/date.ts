...
import { DATE_THRESHOLDS } from '@/constants/layout';
...
export const formatPaymentDueDate = (
  ...
  try {
    ...
    // Formato relativo usando constantes
    let relative: string;
    if (diffDays < 0) {
      relative = 'Vencido';
    } else if (diffDays === 0) {
    ...
    } else if (diffDays <= DATE_THRESHOLDS.SOON_DAYS) {
      relative = `En ${diffDays} días`;
    } else {
      // Para cualquier día mayor, mostrar el número de días
      relative = `En ${diffDays} días`;
    }
