...
describe('useStatementExport', () => {
  ...
  describe('exportToPdf', () => {
    it('should set isExporting to true while exporting', async () => {
      ...
      (ExpoPrint.printToFileAsync as jest.Mock).mockImplementation(async () => {
        // Capture state at the moment of export
        capturedIsExporting = true;
      ...
      // After export, isExporting should be false again
      expect(result.current.isExporting).toBe(false);
    ...
    it('should call shareAsync with correct mime type', async () => {
      ...
      expect(ExpoSharing.shareAsync).toHaveBeenCalledWith(expect.stringContaining('EstadoCuenta'), {
        UTI: '.pdf',
  ...
  describe('error handling', () => {
    it('should show alert on error', async () => {
      ...
      expect(mockAlert).toHaveBeenCalledWith('Error', 'No se pudo generar el estado de cuenta');
    });
