...
/** Intervalo de actualización del temporizador (ms) */
...
export default function DynamicCvvScreen() {
  ...
  const [timeLeft, setTimeLeft] = useState(TIMING.DYNAMIC_CVV_EXPIRY_SECONDS);
  ...
  const onBiometricSuccess = () => {
    ...
    setTimeout(() => {
      ...
      // Generar CVV de 3 dígitos (100-999)
      const generatedCvv = Math.floor(100 + Math.random() * 900).toString();
      setCvv(generatedCvv);
      setTimeLeft(TIMING.DYNAMIC_CVV_EXPIRY_SECONDS);
