...
import { AccountCard, AmountInput, InfoBox, TermGrid } from '@/components/cards/operations/advance-components';
...
export default function AdvanceScreen() {
  ...
  useEffect(() => {
    if (id) getCardById(id).then(c => { setCard(c); setIsLoadingCard(false); });
  }, [id, getCardById]);
  ...
  const calculations = useMemo(() => {
    ...
    return { val, interest, total, monthly: total / selectedTerm };
  }, [amount, selectedTerm]);

  const handleExpiryChange = (text: string) => {
    const cleaned = text.replace(/[^0-9]/g, '');
    const formatted = cleaned.length >= 2 ? cleaned.substring(0, 2) + '/' + cleaned.substring(2, 4) : cleaned;
    setExpiry(formatted);
  ...
  if (isLoadingCard) return <LoadingScreen message="Cargando opciones de avances..." />;
  if (isProcessing) return <LoadingScreen message="Procesando avance de efectivo..." />;
    ...
    <ThemedView style={styles.container} surface={1}>
      <Animated.View exiting={SlideOutLeft} style={{ flex: 1 }}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <CardOperationHeader title="Avance de Efectivo" card={card} onBack={handleBack} isModal={step === 'form'} />
            ...
            <Animated.View key={step} entering={direction === 'forward' ? SlideInRight.duration(300) : SlideInLeft.duration(300)} exiting={direction === 'forward' ? SlideOutLeft.duration(300) : SlideOutRight.duration(300)} style={styles.stepContainer}>
              {step === 'form' && (
                <>
                  ...
                  <TermGrid terms={TERMS} selectedTerm={selectedTerm} onSelectTerm={setSelectedTerm} />
                  <ThemedText style={styles.sectionTitle}>Cuenta de destino</ThemedText>
                  {ACCOUNTS.map(acc => <AccountCard key={acc.id} account={acc} isSelected={selectedAccount === acc.id} onSelect={() => setSelectedAccount(acc.id)} />)}
                </>
              )}
              {step === 'summary' && (
                <>
                  <SummaryPanel title="Resumen de la solicitud" items={[
                  ...
                  <View style={[styles.table, { borderColor: theme.colors.border }]}>
                    ...
                    {selectedTerm > 5 && <View style={styles.tableRow}><ThemedText style={{ opacity: 0.5, fontStyle: 'italic' }}>... {selectedTerm - 5} cuotas más</ThemedText></View>}
                  </View>
                </>
              )}
              {step === 'validation' && (
                <>
                  ...
                  <ThemedText style={{ opacity: 0.7, marginBottom: 24 }}>Ingresa los datos de tu tarjeta para confirmar.</ThemedText>
                  <View style={styles.formGroup}>
                    <ThemedText style={styles.label}>Fecha de Expiración (MM/YY)</ThemedText>
                    <TextInput style={[styles.inputField, { borderColor: theme.colors.border, color: theme.colors.text, backgroundColor: theme.colors.surface }]} placeholder="MM/YY" placeholderTextColor={theme.colors.textDisabled} value={expiry} onChangeText={handleExpiryChange} maxLength={5} keyboardType="number-pad" returnKeyType="next" blurOnSubmit={false} onSubmitEditing={() => cvvRef.current?.focus()} inputAccessoryViewID="uniqueID" />
                  </View>
                  <View style={styles.formGroup}>
                    <ThemedText style={styles.label}>Código de Seguridad (CVV)</ThemedText>
                    <TextInput ref={cvvRef} style={[styles.inputField, { borderColor: theme.colors.border, color: theme.colors.text, backgroundColor: theme.colors.surface }]} placeholder="123" placeholderTextColor={theme.colors.textDisabled} value={cvv} onChangeText={setCvv} maxLength={4} secureTextEntry keyboardType="number-pad" returnKeyType="done" inputAccessoryViewID="uniqueID" />
                  </View>
                  <InfoBox message="Al continuar, aceptas los términos y condiciones del servicio de Avance de Efectivo." />
                </>
              )}
            </Animated.View>
          ...
          <View style={[styles.footer, { backgroundColor: theme.colors.surface, paddingBottom: insets.bottom + 20 }]}>
            <TouchableOpacity style={[styles.button, { backgroundColor: theme.tenant.mainColor, opacity: (step === 'form' && !amount) ? 0.5 : 1 }]} onPress={handleNext} disabled={step === 'form' && !amount}>
              <ThemedText style={styles.buttonText}>{step === 'validation' ? 'Confirmar y Acreditar' : 'Continuar'}</ThemedText>
      ...
      <BiometricGuard isVisible={showBiometrics} onSuccess={onBiometricSuccess} onCancel={() => setShowBiometrics(false)} reason="Confirma el avance de efectivo" />
...
const styles = StyleSheet.create({
  ...
  tableRow: { flexDirection: 'row', padding: 12, borderBottomWidth: 1, borderBottomColor: 'transparent' },
  col1: { width: '20%', textAlign: 'center' },
  col2: { width: '40%', textAlign: 'right' },
  col3: { width: '40%', textAlign: 'right' },
  formGroup: { gap: 8 },
  label: { fontSize: 14, opacity: 0.8 },
  inputField: { height: 50, borderWidth: 1, borderRadius: 12, paddingHorizontal: 16 },
  footer: { padding: 20, paddingBottom: 40, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.05)' },
  button: { height: 56, borderRadius: 28, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 4.65, elevation: 8 },
