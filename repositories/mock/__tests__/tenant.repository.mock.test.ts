...
jest.mock('@/api/config', () => ({
  API_CONFIG: {
    MOCK_DELAY: 0, // No delay for tests
  },
}));
...
describe('MockTenantRepository', () => {
  ...
  describe('getTenants', () => {
    ...
    it('should return a copy of tenants array', async () => {
      const tenants1 = await repository.getTenants();
      const tenants2 = await repository.getTenants();
  ...
  describe('getTenantById', () => {
    ...
    it('should return undefined for non-existent ID', async () => {
      const tenant = await repository.getTenantById('99999');
  ...
  describe('getTenantBySlug', () => {
    it('should return tenant by slug', async () => {
      const tenant = await repository.getTenantBySlug('bpichincha');
    ...
    it('should return undefined for non-existent slug', async () => {
      const tenant = await repository.getTenantBySlug('nonexistent-slug');
  ...
  describe('searchTenants', () => {
    it('should return all tenants for empty query', async () => {
      const allTenants = await repository.getTenants();
      const searchResults = await repository.searchTenants('');

      expect(searchResults.length).toBe(allTenants.length);
    });

    it('should search by name', async () => {
      const tenants = await repository.searchTenants('Pichincha');
    ...
    it('should be case-insensitive', async () => {
      const lowerCase = await repository.searchTenants('ecuador');
      const upperCase = await repository.searchTenants('ECUADOR');
    ...
    it('should return empty array for no matches', async () => {
      const tenants = await repository.searchTenants('zzzznonexistent');

      expect(tenants).toEqual([]);
    });

    it('should search partial names', async () => {
      const tenants = await repository.searchTenants('banco');
